        ...
        <td>
            <div class="dropdown">
                ...
                <div class="dropdown-menu">
                    @if ($status == 'index')
                        <a class="dropdown-item" href="{{ route('admin.attribute.edit', $item->id) }}">
                            <i class="ti ti-pencil me-1"></i> Edit
                        </a>
                        <form action="{{ route('admin.attribute.delete', $item->id) }}" method="post">
                            @csrf
                            @method('delete')
                    ...
                    @else
                        <form action="{{ route('admin.attribute.restore', $item->id) }}" method="post">
                            @csrf
                            <button class="dropdown-item">
                                <i class="ti ti-repeat me-1"></i> Restore
                            </button>
                        </form>
                        <form action="{{ route('admin.attribute.destroy', $item->id) }}" method="post">
                            @csrf
                            @method('delete')
