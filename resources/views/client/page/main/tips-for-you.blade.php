...
@php
    ...
    if ($productmenus->isNotEmpty()) {
        ...
        if ($secondProductMenu->productmenus->isNotEmpty()) {
            $productmenuitemsecond = $secondProductMenu->productmenus->first()->productmenuitems;
        }
    }
@endphp
{{-- {{$productmenus->skip(1)->first()->productmenus->first()->productmenuitems}} --}}

@if ($productmenuitemsecond)
    <section>
        <div class="container">
            <h4 class="fw-bold py-4">{{$productmenus->skip(1)->first()->productmenus->first()->name}}</h4>
            <div class="bg-white rounded-4 p-2">
                <div class="row">
                    @foreach ($productmenuitemsecond as $index => $categories)
