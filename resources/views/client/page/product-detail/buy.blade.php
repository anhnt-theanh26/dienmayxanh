<div class="col-5">
    <div class="bg-white rounded-3 p-3">
        ...
        <div class="product">
            @php
                $publishedVariants = $product->variants->where('status', 'published')->values();
            @endphp
            <div id="nav-tab" role="tablist">
                @foreach ($publishedVariants as $variant)
                    @if ($variant->status == 'published')
                        <button class="btn btn-outline-primary {{ $loop->first ? 'active' : '' }}"
                            @if ($variant->status == 'draft') disabled @endif
...
<script>
    function addtocart(id) {
        $.ajax({
                url: "{{ route('cart.add-to-cart', ['id' => ':id']) }}".replace(':id', id),
                type: "GET",
            })
            .done((response) => {
